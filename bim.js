...
const buildingData = {
    ...
    coworking: {
        ...
        content: `<ul class="space-y-4">
                    ...
                    <li class="flex items-start gap-3"><div class="mt-1 p-1 bg-primary/10 rounded text-primary"><i data-lucide="door-open" class="w-4 h-4"></i></div><span>Sas d'entrée "à la japonaise" (vestiaire) pour réduire l'entretien</span></li>
                </ul>`,
        icon: "briefcase"
    },
    rdc: {
    ...
    basement: {
        ...
        content: `<ul class="space-y-4">
                    <li class="flex items-start gap-3"><div class="mt-1 p-1 bg-primary/10 rounded text-primary"><i data-lucide="hammer" class="w-4 h-4"></i></div><span>Fablab complet (Mécanique, Bois, Élec) pour l'entretien et la production</span></li>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (slider) {
        slider.addEventListener('input', (e) => {
            document.documentElement.style.setProperty('--explode-gap', `${e.target.value}px`);
        });
    }
});
